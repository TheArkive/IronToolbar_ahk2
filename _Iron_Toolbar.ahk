...
class gui_toolbar extends Gui {
    Static __New() {
        super.Prototype.AddToolbar := ObjBindMethod(this,"AddToolbar")
    }
    Static AddToolbar(gui, p*) {
        return Toolbar.AddToolbar(gui, p*)
    }
}
...
class Toolbar { ; extends Toolbar.Private {
    ...
    Static AddToolbar(in_gui:="", sOptions:="", Styles:="", MixedBtns := true, EasyMode := true) { ; TBSTYLE_FLAT := 0x0800 Required for separators as bars.
        _Styles := "", startStyles := Styles, pos := "top", exStyles := ""
        ...
        If easyMode { ; final easy mode modifications
            (!InStr(Styles,"NoParentAlign")) ? (_Styles .= " " this.lu("NoParentAlign",true), startStyles .= " NoParentAlign") : ""
            (!InStr(Styles,"NoResize")) ? (_Styles .= " " this.lu("NoResize",true), startStyles .= " NoResize") : ""
            (!InStr(Styles,"Flat")) ? (_Styles .= " " this.lu("Flat",true), startStyles .= " Flat") : ""
            (MixedBtns And !InStr(Styles,"List")) ? (_Styles .= " " this.lu("List",true), startStyles .= " List") : ""
            (MixedBtns And !InStr(Styles,"MixedButtons")) ? exStyles .= "MixedButtons " : ""
            startStyles := RegExReplace(startStyles,"i)(left|right|top|bottom)","")
            startStyles := Trim(RegExReplace(startStyles,"[ ]{2,}"," "))
            
            For prop, value in Toolbar.exStyles.OwnProps() ; filter out exStyles from main startStyles
                startStyles := RegExReplace(StrReplace(startStyles,prop,""),"[ ]{2,}"," ")
        }
        ...
        ctl.ShowTextStatus := true, ctl.MixedBtns := MixedBtns, ctl.easyMode := easyMode, ctl.pos := pos
        ...
        For prop, value in Toolbar.OwnProps() { ; attach all ctl_* methods as native methods to the ctl
            if RegExMatch(prop, "^ctl_") {
                _name := RegExReplace(prop,"^ctl_","")
                ctl.%_name% := ObjBindMethod(this, prop)
            }
        }
        ctl.DefineProp("Type",{Get: (_type)=>("Toolbar")}) ; redefine .Type property
        
        this.ResetStructs(ctl)
        result := ctl.SendMsg(this._SetExtendedStyle, 0, this.MakeFlags(exStyles,"exStyles")) ; set exStyles
        
        ; this.SendMsg(ctl, this._SetButtonWidth,0,this.MakeLong(16,200))
        return ctl
    }
    ...
    Static lu(sInput,TextOutput := false) { ; LookUp - return integer
    ...
    Static ctl_AutoSize(ctl) {
        PostMessage this._AutoSize, 0, 0, ctl.hwnd ; no return value, and no more ErrorLevel... hm...
    }
    
    Static ctl_Add(ctl, btnArray, initial:=true) {
        If initial
            btnArray := this.AddConvert(ctl,btnArray,initial)
        
        TBBUTTON := BufferAlloc(Toolbar.TBBUTTON_size * btnArray.Length, 0)
        offset := 0
        
        For i, b in btnArray ; add buttons
            this.Fill_TBBUTTON(ctl, TBBUTTON, offset, b.label, b.icon, b.idCmd, b.states, b.styles, b.iString), offset += Toolbar.TBBUTTON_size
        
        result := ctl.SendMsg(this._AddButtons, btnArray.Length, TBBUTTON.ptr)
        ctl.SendMsg(this._SetMaxTextRows,ctl.ShowTextStatus ? 1 : 0) ; set text display mode
        
        If (ctl.easyMode And initial)
            ctl.Position(ctl.pos), ctl.Position(ctl.pos)
         
        ctl.AutoSize() ; , this.reAddButton := true
    }
    
    Static ctl_ClearButtons(ctl) {
        While ctl.SendMsg(this._DeleteButton, 0, 0)
            i := 1
    }
    
    Static ctl_CmdToIdx(ctl, idCmd) {
        return (ctl.SendMsg(this._CommandToIndex, idCmd) + 1) << 32 >> 32
    }
    
    Static ctl_Count(ctl) {
        return ctl.SendMsg(this._ButtonCount)
    }
    
    Static ctl_Customizer(ctl) {
        files := ctl.ImageLists[ctl.IL_Default].files ; duplicate image list
        large := ctl.ImageLists[ctl.IL_Default].large, ctl.IL_Create("Customizer",files,large)
        
        events := ObjBindMethod(this,"CustoEvents", ctl)
        custo := Gui("AlwaysOnTop -MinimizeBox -MaximizeBox Owner" ctl.gui.hwnd,"Customizer")
        LV := custo.Add("ListView","w200 h200 vCustoList Report Checked -hdr",["Icon List"])
        LV.OnEvent("ItemCheck",events)
        LV.ModifyCol(1,170)
        LV.SetImageList(ctl.ImageLists["Customizer"].handle,1)
        ...
        custo.btnsReset := this.SaveNewLayout(ctl)
        
        custo.Show("hide")
        ctl.gui.GetPos(&x,&y,&w,&h), cX := x+(w//2), cY := y+(h//2)
        custo.GetPos(,,&w,&h)
        x := cX-(w//2), y := cY-(h//2)
        custo.Show("x" x " y" y)
    }
    
    Static ctl_Delete(ctl, idx) {
        return ctl.SendMsg(this._DeleteButton, idx-1,0)
    }
    
    Static ctl_EnableButton(ctl, idx, status) {
        btns := this.SaveNewLayout(ctl)
        result := ctl.SendMsg(this._EnableButton,btns[idx].idCmd,this.MakeLong(status,0))
        return result
    }
    
    Static ctl_Export(ctl) {
        btns := this.SaveNewLayout(ctl), a := []
        For i, b in btns {
    ...
    Static ctl_HideButton(ctl, idx, status) {
        btns := this.SaveNewLayout(ctl)
        result := ctl.SendMsg(this._HideButton,btns[idx].idCmd,this.MakeLong(status,0))
        return result
    }
    
    Static ctl_IL_Add(ctl, ILname, icons) { ; append icons to an existing imagelist
        If !ctl.ImageLists.Has(ILname)
            throw Exception("Image list name doesn't exist: " ILname)
        handle := ctl.ImageLists[ILname]
        For i, o in icons {
            a := StrSplit(o,"/"), img := a[1], icon := (a.Has(2)) ? a[2] : ""
            If (icon!="")
                IL_Add(handle,img,icon)
            Else IL_Add(handle,img)
            ctl.ImageLists[ILname].files.Push(o) ; append icons to file list
        }
    }
    
    Static ctl_IL_Create(ctl, ILname, icons, large:=false) {
        ...
        ctl.ImageLists[ILname] := {handle:handle, files:icons, large:large}
        return true ; all icons added successfully
    }
    
    Static ctl_IL_Destroy(ctl, ILname) {
        handle := ctl.ImageLists[ILname].handle
        ctl.ImageLists.Delete(ILname)
        return IL_Destroy(handle)
    }
    
    Static ctl_Import(ctl,a) {
        r := []
        For i, btn in a {
            b := {}
    ...
    Static ctl_Insert(ctl,b,idx) {
        a := this.AddConvert(ctl, b, true), b := a[1]
        TBBUTTON := BufferAlloc(Toolbar.TBBUTTON_size, 0)
        this.Fill_TBBUTTON(ctl,TBBUTTON, offset:=0, b.label, b.icon, b.idCmd, b.states, b.styles, b.iString)
        ctl.SendMsg(this._InsertButton, idx-1, TBBUTTON.ptr)
    }
    
    Static ctl_IsChecked(ctl,idx,int := false) {
        TBBUTTON := BufferAlloc(Toolbar.TBBUTTON_size,0)
        r := ctl.SendMsg(this._GetButton, idx-1, TBBUTTON.ptr)
        states := NumGet(TBBUTTON,8,"Char"), TBBUTTON := ""
        return !!(states & Toolbar.states.Checked) ; only return 1 or 0
    }
    
    Static ctl_MoveButton(ctl,idx,pos) {
        result := ctl.SendMsg(this._MoveButton, idx-1, pos-1)
        return result
    }
    
    Static ctl_OldCustomize(ctl) {
        ctl.SendMsg(this._Customize, 0, 0)
    }
    
    Static ctl_Position(ctl, p:="", repeat:=false) {
        ctl.pos := p
        dims := this.GetButtonDims(ctl) ; bWidth, bHeight, hPad, vPad
        ctl.gui.GetClientPos(,,&w,&h)
        height := dims.h+2, width := dims.w+2
        
        If (p="right") {
            ctl.Move(w-width,0,width,h)
            ctl.SendMsg(this._SetStyle, 0, this.MakeFlags(ctl.startStyles " Vert","styles"))
        } Else If (p="bottom") {
            ctl.Move(0,h-height,w,height)
            ctl.SendMsg(this._SetStyle, 0, this.MakeFlags(ctl.startStyles,"styles"))
        } Else If (p="left") {
            ctl.Move(0,0,width,h)
            ctl.SendMsg(this._SetStyle, 0, this.MakeFlags(ctl.startStyles " Vert","styles"))
        } Else { ; top
            ctl.Move(0,0,w,height)
            ctl.SendMsg(this._SetStyle, 0, this.MakeFlags(ctl.startStyles,"styles"))
        }
    ...
    Static ctl_SendMsg(ctl, msg, wParam:=0, lParam:=0) { ; for generic message sending
        return SendMessage(msg, wParam, lParam, ctl.hwnd)
    }
    
    Static ctl_SetButtonSize(ctl, W, H) {
        Long := this.MakeLong(W, H)
        result := ctl.SendMsg(this._SetButtonSize, 0, Long)
        ctl.Move(,,,H)
        ; this.Position(this.pos), this.Position(this.pos)
        return result
    }
    
    Static ctl_SetImageList(ctl, Default, Hot := "", Pressed := "", Disabled := "") {
        ctl.IL_Default := Default
        ctl.IL_Hot := Hot
        ctl.IL_Pressed := Pressed
        ctl.IL_Disabled := Disabled
        
                     result := ctl.SendMsg(this._SetImageList,         0, ctl.ImageLists[Default].handle)
        (Hot)      ? result := ctl.SendMsg(this._SetHotImageList,      0, ctl.ImageLists[Hot].handle) : ""
        (Pressed)  ? result := ctl.SendMsg(this._SetPressedImageList,  0, ctl.ImageLists[Pressed].handle) : ""
        (Disabled) ? result := ctl.SendMsg(this._SetDisabledImageList, 0, ctl.ImageLists[Disabled].handle) : ""
        ctl.Position(ctl.pos), ctl.Position(ctl.pos)
        
        return result
    }
    
    Static ctl_ShowText(ctl, status) {
        ctl.ShowTextStatus := status
        ctl.SendMsg(this._SetMaxTextRows, status)
        ctl.Position(ctl.pos) ; 2x redraws required for proper dimensions
        ctl.Position(ctl.pos)
    }
    ...
    Static Fill_TBBUTTON(ctl, TBBUTTON, offset, label, iBitmap, idCommand, fsState, fsStyle, iString:=-1) {
        ...
        If (iString = -1) { ; create string and add to pool
            ...
            iString := ctl.SendMsg(this._AddString, 0, BTNSTR.ptr)
        }
    ...
    Static GetButtonText(ctl, idx) {
        ...
        tSize := ctl.SendMsg(this._GetButtonText, idCmd, 0) << 32 >> 32 ; needed for 32-bit compatibility
        
        If (tSize = -1)
            return ""
        
        TBBUTTON := "", tBuf := BufferAlloc((tSize+1) * (StrLen(Chr(0xFFFF)) ? 2 : 1),0)
        ctl.SendMsg(this._GetButtonText, idCmd, tBuf.Ptr), txt := StrGet(tBuf)
        return txt
    }
    Static GetButtonDims(ctl) {
        ...
        For i, b in btns {
            ...
            dwMask := this.Style
            NumPut "UInt", bSize, "Int", dwMask, TBI
            ctl.SendMsg(this._GetButtonInfo, b.idCmd, TBI.ptr)
            styles := NumGet(TBI,17,"Char")
            If (styles & Toolbar.bStyles.Sep) ; check and skip if separator
                continue
            
            r := ctl.SendMsg(this._GetRect, b.idCmd, _RECT.ptr)
        ...
        padding := ctl.SendMsg(this._GetPadding, 0, 0), this.MakeShort(padding, &hPad, &vPad)
        dims := {w:bWidth, h:bHeight, hPad:hPad, vPad:vPad}, TBI := "", _RECT := ""
        return dims
    }
    
    Static __tbNotify(ctl, lParam) { ; TB_HITTEST / TB_GETRECT/TB_GETITEMRECT ; https://docs.microsoft.com/en-us/windows/win32/controls/bumper-toolbar-control-reference-notifications
        ...
        Switch ctl.NMHDR.code { ; WM_NOTIFY msgs ; https://docs.microsoft.com/en-us/windows/win32/controls/bumper-toolbar-control-reference-notifications
        ...
        If (o.idCmd) {
            ...
            r := ctl.SendMsg(this._GetRect, b.idCmd, RECT.ptr)
    ...
    Static SaveNewLayout(ctl) { ; works, but not keeping track of removed buttons yet
        btns := []
        Loop ctl.SendMsg(this._ButtonCount) {
            b := this.GetButton(ctl, A_Index) ; idCmd, iImg, states, styles, width, txt
    ...
    Static CustoEvents(_ctl,ctl,p*) { ; _ctl is the toolbar control
        ...
        ; msgbox "ctl1: " Type(_ctl) " / ctl2: " Type(ctl)
        ...
        If (ctl.Name = "MoveUp" Or ctl.Name = "MoveDown") {
            ...
            If LV.GetText(r) != "Seperator"
                return
            _ctl.SendMsg(this._DeleteButton, r-1)
